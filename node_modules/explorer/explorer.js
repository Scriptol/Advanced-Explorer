...
async function sendToTrash(p) { 
  try {
    ...
    if (substTarget) {
      // normalized ex. = "E:\\folder\\file.txt"
    ...
    if(debug) console.log("[trash] safePath:", safePath);
  ...
  for (let nextfile of flist) {
    ...
    if(stats.isDirectory())  {
      ...
      if (!fs.existsSync(targetpath)) {
        ...
        /*
        const delta = Math.abs(stats.mtime.getTime() - tstat.mtime.getTime());
        if (delta > 2000) {
          newer = true;
        }
        */  
...
function synchronize(params) {
  ...
  if (abortRequested) {
    messageToInterface({content: "Synchronization stopped by the user" });
  }  
