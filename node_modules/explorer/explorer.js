...
function getSubstDriveTarget(drive) {
  ...
  console.log(`[subst] no mapping for ${cleanDrive}`);
  return null;
}
...
async function sendToTrash(p) { 
  try {
    ...
    if (isSDCard(drive)) {
      ...
      console.log("[trash] Deleted permanently (removable drive)");
      return true;
    }

    const realPath = fs.realpathSync(normalized);
    const safePath = path.resolve(realPath);
    await shell.trashItem(safePath);
    if(debug) console.log("[trash] Item successfully moved to trash");
    return true;
  } 
  catch (error) {
...
function copySync(sourcename, targetname, refresh) {
    ...
    let readHandle = fs.openSync(sourcename, 'r');
    let writeHandle = fs.openSync(targetname, 'w');
    let size;
    let seekoffset = 0;
    try {
        do  {
...
function copysub(flist, sourcedir, targetdir) {
  ...
  for(let i = 0; i < flist.length; i++)  {
    ...
    var stats = fs.statSync(sourcename);
    if(stats.isDirectory()) {
      if(!fs.existsSync(targetname)) {
        try {
      ...
      var sublist = fs.readdirSync(sourcename);
...
function copyOne(params) {
    ...
    messageToInterface("Copying "+ name + " to " + targetdir); 
    ...
    updateTarget(params.target)
    messageToInterface(name + " copied"); 
}
...
function filecopy(params) {
  ...
  messageToInterface(selection.length + ' file' + s + ' copied.');
  perr(selection.length + " files copied.");
...
function sendForComputer(data) {
  event.sender.send("computer", data);
}
...
function sendDriveList(letter) {
    ...
    if (platform !== "win32") {
        const drives = [ "/"];
        sendForComputer(JSON.stringify({letter: letter, drives }));
        return;
    }
    ...
    exec('powershell -command "Get-PSDrive -PSProvider FileSystem | Select-Object -ExpandProperty Name"', 
          (err, stdout) => {
            if (err) {
              sendForComputer(JSON.stringify({type: "ERROR", message: "Unable to list drives" }));
              return;
            }
