...
{
   ws.emit(JSON.stringify( { "type":"message",  'content' : content } ));
}
...
function waitForAnswer(code) {
    ...
    var ctr = 0;
    while(data == '?') setTimeout(function() {
        ctr++;
        if(ctr>10) {
          ctr = 0;
          console.log("Waiting...");
	...
	fs.exists(fullpath, function(here)
	{
		if(here)
		{
    	  ...
    	  if(stats.isDirectory())
    	  {
        	 websocket.send(JSON.stringify({
            "type":'message',  
            'content': "Can't edit " + fullpath + ", it is a directory." } ));
        	 return;
      	  }
...
function savefile(websocket, fs, params)
{
  ...
  if(!params.overwrite)
  {
    if(fs.existsSync(fullpath))
    {
      ...
      var data = waitForAnswer("alreadyexists");
...
function isempty(websocket, fs, params)
{
  ...
  fs.exists(fullpath, function (r) {
    if(!r) {
      websocket.send(JSON.stringify({
        "type":'message', 'content' : "Dir not found " + fullpath }));
      return;
    }
    fs.readdir(thepath, function(err, arrfiles)
	  {
       websocket.send(JSON.stringify({
        "type":'message', 'content' : arrfile.length }));
    });
  });
}
...
function textinzip(websocket, fs, params)
{
  ...
  if(zipEntry != null && zipEntry != false)
  {
    ...
    } catch(e)
    {
      websocket.send(JSON.stringify({
        "type":'message', 
        'content':'Enable to extract. See the issues page.' 
      }));
      return;
    }
...
exports.shell = function(websocket, fs, params)
{
	...
	case 'filecopy':
		...
		break;
  case 'copyover':
    copyIf(websocket, filepath, target, false);
    break;
  case 'copyzip':
  ...
  case 'extract':
	...
	default:
		  websocket.send(JSON.stringify({
        "type":'message', 
        "content": 'Unknow command '+command+' to Explorer' 
      }));
	}
}