...
const {runner, exec, execSync } = require('child_process');
const fs = require('fs');
...
exports.abortRequested = abortRequested;
...
function messageToInterface(content, remain=true) {
...
function alertToInterface(content) {
   event.sender.send("interface", JSON.stringify({ "type":"alert", "content" : content }));
}
...
function getdir(params) {
  ...
  // trick for aliases of dirs under Windows

  if(os.platform()=="win32") {
    if(thepath == "/") {
      thepath = "c:/"
    }    
    if(thepath.length==3 && thepath.slice(1,3)===":/") {
      thepath +="."; 
    }
    const driveLetter = thepath[0]
    try {
      fs.readdirSync(driveLetter + ":");
    } 
    catch (e) {
      thepath="c:/"
    }  
  ...
  perr("Getdir currentpath request: " + thepath + " target: " + params.target);  
  ...
  fs.readdir(thepath, function(err, arrfiles)  {
    ...
    for(var i = 0; i < arrfiles.length; i++) {
      ...
      try  {
        ...
        perr("Enable to read " + p);   
      }
	  }

    fs.realpath(thepath, "", function (err, resolvedPath) {
      if (err) throw err;
		  perr("Explorer getdir resolvedPath: " + resolvedPath);
...
function readImage(filepath) {
  ...
  perr("Launching request for image: " + filepath);
  if(fs.existsSync(filepath)) {
    const image = fs.readFileSync(filepath);
    perr("Request for image completed.");
    sendToInterface({
        "type":"image", 
        "ext": ext, 
        "path": filepath,
...
function dirup(target) {
  ...
  if(p === "/" || (p[1]===":" && p[2]==="/" && p.length==3)) {
    return true;      // already in root, exit
  }  
  p = p.replace(/\\/gi, "/");
  p = p.replace(/\/\//gi, "/");
...
function chDrive(filepath, target) {
  if(filepath.charAt(0) == "/") {
    return setpath(filepath, target);
  }
  let path = filepath.slice(0, 2) + "/"
  currpath[target] = path;
  return true;
}
...
function rename(params) {
  ...
  let oldname = path.join(currpath[target], params.oldname);
  oldname = noHTMLchars(oldname);
  let newname = path.join(currpath[target], params.newname);
  try {
  ...
  for (const name of flist) {
    ...
    if (stats.isDirectory()) {
      ...
      try {
        ...
        totalDeleted++;      // fichier supprimé
      } catch (e) {
        errorcount++;     // erreur de suppression
      }
    }
  }
...
function getSubstDriveTarget(drive) {
  // drive : "E:" or "C:"
  const cleanDrive = drive.trim().replace(':', '').toUpperCase() + ":";

  try {
    const output = execSync("subst", { encoding: "utf8" });
    if(debug) console.log("[subst] output:\n" + output);

    for (const line of output.split(/\r?\n/)) {
      if (!line.trim()) continue;

      // Example : "P:\: => C:\Dev\Project"
      const parts = line.split(" => ");
      if (parts.length !== 2) continue;

      const left = parts[0].trim().toUpperCase();   // "E:\:"
      const right = parts[1].trim();                // "C:\\Dev\\Project"

      // Format "E:"
      if (left.startsWith(cleanDrive)) {
        console.log(`[subst] ${cleanDrive} mapped to ${right}`);
        try {
          const real = fs.realpathSync(right);
          console.log(`[subst] realpathSync(${right}) = ${real}`);
          return real;
        } catch {
          return right;
        }
    ...
    console.log("[subst] execSync error:", e.message);
  }

  perr(`[subst] no mapping for ${cleanDrive}`);
  return null;
}


function isRealDrive(driveLetter) {
  try {
    return fs.existsSync(driveLetter + "\\");
  } catch {
    return false;
  }
}
...
function hasRecycleBin(driveLetter) {
  const recyclePath = driveLetter + "\\$Recycle.Bin";
  try {
    return fs.existsSync(recyclePath);
  } catch {
    return false;
  }
}
async function sendToTrash(p) { 
  try {
    const normalized = path.win32.normalize(p);
    const drive = normalized.slice(0, 2).toUpperCase(); // "M:"

    const substTarget = getSubstDriveTarget(drive);
    if (substTarget) {
      const rest = normalized.slice(2);
      const safePath = path.resolve(path.win32.join(substTarget, rest));
      await shell.trashItem(safePath);
      perr("[trash] Item successfully moved to trash (SUBST)");
      return true;
    }

    if (!isRealDrive(drive)) {
      fs.unlinkSync(normalized);
      perr("[trash] Deleted permanently (not a real drive)");
      return true;
    }

    if (hasRecycleBin(drive)) {
      await shell.trashItem(normalized);
      perr("[trash] Item successfully moved to trash (real drive with recycle bin)");
      return true;
    }

    // Real drive no trash → SD, USB FAT32/exFAT
    deleteFileDir(normalized);
    console.log("Deleted permanently (real drive without recycle bin)");
    return true;

  } catch (error) {
    console.log("[trash] Not moved to trash:", error.message);
    return false;
  }
}
...
function finalizeDelete(target) {
  let message =  totalDeleted + " deleted";
...
async function deletelist(params) {
  const target = params.target;
  let flist = params.list;
  let fullflag = params.fullpath;
  totalDeleted = 0;
  errorcount = 0;
  let p;
  ...
  if(os.platform() != "win32") {
    for(let i = 0; i < flist.length;i++)   {
      ...
      if(fullflag)
        ...
        p = path.join(currpath[target], flist[i]);
      deleteFileDir(p)
    }  
  ...
  let realTarget = false;
  let drivePrefix = ""

  for (let i = 0; i < flist.length; i++) {
    if (!flist[i]) continue;
    if(fullflag) 
      p = path.win32.normalize(flist[i]);
    else
      p = path.win32.normalize(path.join(currpath[target], flist[i]));
    
    if(!realTarget) {
      const driveLetter = p[0];
      drivePrefix = driveLetter.toUpperCase() + ':';
      realTarget = getSubstDriveTarget(drivePrefix);
    }

    let targetPath = p;
    if (realTarget && p.toUpperCase().startsWith(drivePrefix)) {
      let relativePath = p.slice(drivePrefix.length);
      targetPath = path.win32.join(realTarget, relativePath);
    }

    const ok = await sendToTrash(targetPath);
    if (ok) totalsize++;
    else errorcount++;
...
/*
function deleteSync(params) {
  var flist = params.files;
  totalfile = 0;
  errorcount = 0;
  let p;
...
function copySync(sourcename, targetname, refresh) {
    try {
        fs.copyFileSync(sourcename, targetname);
        totalCopied++
    } catch(e) {
        console.log("Error, enable to copy " + sourcename + ", " + e.message);
        copyErrors++
    }

    let txt = sourcename + " copied";
    if(copyErrors > 0) txt = sourcename + " not copied"
    perr(txt);
    messageToInterface(txt);     
    if(refresh) {
        sendToInterface({ 'type':'notification', 'action':'differed' });  
    }
    return;
}


function copyIf(sourcename, targetname, promptdup) {
  if(promptdup)   {
    if(fs.existsSync(targetname))   {
      sendToInterface({
        "type":"confirm",
        "question" : targetname + " already exists. Overwrite?",
        "command": "copyover",
        "path" : sourcename,
        "tpath" : targetname, 
      });
  ...
  copySync(sourcename, targetname, false);
  return;
};



function copysub(flist, sourcedir, targetdir) {

  for(let i = 0; i < flist.length; i++)  {
    let nextfile = flist[i];
    if(nextfile=='') continue;
    let sourcename = path.join(sourcedir, nextfile);
    sourcename = noHTMLchars(sourcename);
    let targetname = path.join(targetdir, nextfile);
    
    const info = "Copying "+ sourcename + " to " + targetdir;
    perr(info);
    messageToInterface(info, true);
    ...
    if(stats.isDirectory()) {
      if(!fs.existsSync(targetname)) {
        try {
          ...
          console.log(targetname+ " not created: " + e.message)
        }
      }
      const sublist = fs.readdirSync(sourcename);
      copysub(sublist, sourcename, targetname);
    }
...
function copyOne(params) {
    let sourcename = params.sourcename
    let targetname = params.targetname
    totalCopied = 0
    copyErrors = 0

    perr("Copying " + sourcename + " to " + targetname)

    let stats = fs.statSync(sourcename);
    if(stats.isDirectory()) {
      if(!fs.existsSync(targetname)) {
      ...
      let sublist = fs.readdirSync(sourcename);
      copysub(sublist, sourcename, targetname);
    }
    else if(stats.isFile()) {
      copySync(sourcename, targetname, false);
...
function showCount() {
  let s = ""; 
  if(totalCopied > 1) s = "s";  
  let txt = totalCopied + ' file' + s + ' copied.'
  if(copyErrors > 0) txt += " " + copyErrors + " not copied."
  setTimeout(() => messageToInterface(txt), 2000);  
  perr(txt);  
}
...
function filecopy(params) {
  ...
  copysub(selection, params.source, params.target);
  ...
  messageToInterface(txt);
  perr(txt);
  updateTarget('rcontent')
}

var win = null;
exports.win = win;

function resolveSubstPathServer(p) {
    let normalized;
    try {
        ...
        return resolveSubstParent(normalized);
    }
}

function resolveSubstParent(p) {
    ...
    try {
        ...
        return resolveDeepParent(p);
    }
}

function resolveDeepParent(p) {
    let current = p;
    while (true) {
        const parent = path.dirname(current);
        if (parent === current) {
            return p; // impossible à résoudre
        }
        try {
            const realParent = fs.realpathSync(parent);
            const rest = p.slice(parent.length);
            return path.join(realParent, rest);
        } catch {
            current = parent;
        }
    }
...
function copyRename(params) {
    ...
    //  SUBST + normalize
    const sourcepath = resolveSubstPathServer(oldname);
    const targetpath = resolveSubstPathServer(newname);

    const txt = `Copying ${sourcepath} to ${targetpath}`
    perr(txt);
    messageToInterface(txt, true)

    if (params.isDirectory) {
        fs.mkdirSync(targetpath, { recursive: true });
        const arrfiles = fs.readdirSync(sourcepath);
        copysub(arrfiles, sourcepath, targetpath);
    } else {
        copyIf(sourcepath, targetpath, true);
    }

    messageToInterface(`${sourcepath} copied to ${targetpath}`);
    updateTarget('rcontent')
}


function synchrosub(source, target, total) {
  const flist = fs.readdirSync(source);
  if(flist.length==0) return total;

  const tlist = fs.existsSync(target) ? fs.readdirSync(target) : [];
  const missingInSource = [];

  for (let tfile of tlist) {
    if (!flist.includes(tfile)) {
      missingInSource.push(path.join(target, tfile));
    }
  }
  ...
  if (missingInSource.length > 0) {
    sendToInterface( { 
      "type": "orphan", 
      "list": missingInSource,
      "dir" : target
    })
  }  

  for (let nextfile of flist) {
    if(nextfile=='') continue;
    ...
    const sourcepath = noHTMLchars(path.join(source, nextfile));
    const targetpath = path.join(target, nextfile);
    const stats = fs.statSync(sourcepath);
    if(stats.isDirectory())  {
      if(!fs.existsSync(targetpath))  {
        fs.mkdirSync(targetpath);
      }  
      total = synchrosub(sourcepath, targetpath, total);
      if (abortRequested) {
        return total;
      }  
    }
      ...
      if (!fs.existsSync(targetpath)) {
        ...
        const ldate = Number(buildDate(stats.mtime))
        const rdate = Number(buildDate(tstat.mtime))
        if(ldate > rdate) {
          newer = true;
        }
        if (stats.size !== tstat.size) {
          newer = true;
        }
      }
    
      if(newer) {
        const info = "Copying "+ sourcepath + " to " + target;
        perr(info);
        sendToInterface({ "type":"status", "content": info });
        copySync(sourcepath, targetpath, false);
        total++;
        sendToInterface({ "type":"counter", "content": total });        
      }  
    }
  }
...
function synchronize(params) {
  ...
  perr("Synchronizing from " + source + " to "+ target);
  let total = synchrosub(source, target,  0);
  if (abortRequested) {
    alertToInterface({content: "Synchronization stopped by the user" });
  }  

  let s = ""; 
  if(total > 1) s = "s";

  const result = total +' file'+s+' copied or updated.';

  updateTarget('rcontent')
  sendToInterface({"type":"status", "content": result });  
  perr(result);
}
...
function getContent(params) {
  ...
  if(stats.isDirectory())  {
	    alertToInterface("Can't edit " + fullpath + ", it is a directory.");
	    return;
  }
...
function openProject(params) {
  ...
  if(obj == null || ! ("type" in obj) || obj.type != "AEPrj") {
        alertToInterface(filename + " is not a valid Advanced Explorer project.")
        return;
  }
...
function updateIni(params) {
  ...
  var fullpath = path.join(root, params.path);  
	var here = fs.existsSync(fullpath);
	if(!here) {
	    perr("Ini file '" + fullpath + "' not found...");
	    return;
	}    
  var data = new String(fs.readFileSync(fullpath));
...
function isempty(params) {
  ...
  fs.readdir(thepath, function(err, arrfiles) {
       messageToInterface(arrfiles.length);
  });
}
...
function dirinfo(params) {
  const  target = params.target;
  const lst = params.filelist;
  let totalsize = 0;
  let totalfile = 0;

  for(var i = 0; i < lst.length; i++)  {
    var fullpath = path.join(currpath[target], lst[i]);
    perr("Next entry " + fullpath);
    var stats = fs.statSync(fullpath);
    if(stats.isDirectory())  {
      infosub(fullpath);
    }
  ...
  if(totalsize > 1000000) {
    const ms = parseInt(totalsize / 1000000, 10);
...
function makeThumbnail(filepath, data) {
  ...
  fs.exists(filepath, function(result) {
    ...
    perr('Command: ' + "php " + filepath + " " + scriptparam);
    var r = runner.exec("php " + filepath + " " + scriptparam,  function(err, stdout, stderr) {
        perr(stdout);
    }); // exec

    perr(filepath + " launched by the server...");
    r.on('exit', function (code) {
      perr('Local script terminated. Sending ' + data.thumbfile + " to the interface.");

    	fs.readFile(data.thumbfile, function(err, file) {
        perr("Request completed.");
        sendToInterface({
          "type":'thumbnail', 
          'params' : data,
...
var explorerWindow = function () {
  ...
  viewWindow.webContents.on('did-finish-load', () => {
    viewWindow.webContents.insertCSS(`body {
      //color: #fff;
      //background: #222;
    }`);
  });  
...
function markdownToHtml(markdown) {
  let html = markdown
    // Titres (ex: # Titre -> <h1>Titre</h1>)
    .replace(/^# (.*$)/gim, '<h1>$1</h1>')
    .replace(/^## (.*$)/gim, '<h2>$1</h2>')
    .replace(/^### (.*$)/gim, '<h3>$1</h3>')
    
    // Gras (ex: **texte** -> <strong>texte</strong>)
    .replace(/\*\*(.*)\*\*/gim, '<strong>$1</strong>')
    
    // Italique (ex: *texte* -> <em>texte</em>)
    .replace(/\*(.*)\*/gim, '<em>$1</em>')
    
    // Liens (ex: [nom](url) -> <a href="url">nom</a>)
    .replace(/\[(.*?)\]\((.*?)\)/gim, "<a href='$2'>$1</a>")
    
    // Sauts de ligne / Paragraphes
    .replace(/^\s*(\n)?(.+)/gm, function(m) {
      return /\<(\/)?(h\d|ul|ol|li|blockquote|pre|img|a)/.test(m) ? m : '<p>' + m + '</p>';
    });
    
    // Auto links
    html = html.replace(/\[(.*?)\]\((.*?)\)/gim, '<a href="$2">$1</a>'); 
    html = html.replace(/(?<!["'])(https?:\/\/[^\s<]+)/gim, '<a href="$1">$1</a>'); 
...
function loadPage(filename) {
    ...
    if (ext === ".md") {
        ...
        let temp = path.join(rootdir, "aexplo" + Date.now() + ".tmp.html");
        let html = markdownToHtml(md);
        fs.writeFileSync(temp, html);
        viewWindow.loadURL(temp);
        viewWindow.webContents.once("did-finish-load", () => {
            viewWindow.show();
            try {
                fs.unlinkSync(temp);
            } catch (e) {}
...
function buildArchive(params) {
    const input = params.list
    const items = Array.isArray(input) ? input : [input];
    const source = params.sourcePath
    const target = params.targetPath
    const zipName = params.zipName
    ...
    items.forEach(item => {
        ...
        if (!fs.existsSync(item)) {
            console.warn("Path not found :", item);
            return;
        }

        if (item.includes(".git")) return;
        ...
        if (stat.isDirectory()) {
            ...
            zip.addLocalFolder(item, folderName, filePath => {
            ...
            zip.addLocalFile(item, "", fileName);
        }
    });

    const outputZipPath = path.join(target, zipName)
...
function viewzip(params) {
  ...
  perr("Zip file: " + zipname);
  try {
    var zipfile = new zip(zipname);
    var zipEntries = zipfile.getEntries();
  } catch(e) {
    alertToInterface('Enable to open '+ zipname + " archive.");
    return;
  }
	var dirlist = [];
  var thedate = null;
...
function extractSub(zipfile, filename, target, keepath, overwrite) {
  ...
  perr("Extracting file: " + filename + " to " + targetPath);
  if(!overwrite)
  {
...
function extract(params) {
  ...
  perr(filelist.length+" to extract. Keep path=" + params.keepath + " Overwrite="+ params.overwrite);
...
function textinzip(params) {
  ...
  if(zipEntry != null && zipEntry != false)  {
    ...
    try {
      ...
      alertToInterface('Enable to extract. See the issues page.');
      return;
    }
    var tempfile = tempdir + path.basename(entryname);
    perr("Loading "+ tempfile);
...
function unzip(params) {
  ...
  perr("Unzip " + zipname + " to " + currpath[target] + " overwrite="+ overwrite);
...
function sendDriveList(letter) {
    let platform = os.platform();
    if (platform !== "win32") {
        const drives = [ "/"];
        sendToInterface({type: "computer", letter: letter, drives });
        return;
    }
    ...
    exec('powershell -command "Get-PSDrive -PSProvider FileSystem | Select-Object -ExpandProperty Name"', 
          (err, stdout) => {
            if (err) {
              sendToInterface({type: "ERROR", message: "Unable to list drives" });
              return;
            }
...
exports.explorerShell = function(params) {
	let command = params.command;
	perr("Explorer command: " + command);
  let target = params.target;
  event = params.event;
  ...
  case 'sync':
        ...
        break;
  case 'copyover':
        copySync(params.srcname, params.tgtname, true);
        break;
  case 'copyone':
        return  copyOne(params);
  ...
  case 'openPrj':
	...
	case 'dirinfo':
		    ...
		    break;
	case 'loadimage':
		    perr("Explorer shell request filepath : " + params.path);
		    readImage(params.path);
		    break;
	case 'thumbnail':
		    perr("Building thumbnail of " + params.imagefile)
  ...
  case 'run':
	...
	default:
        progressOff();
		    alertToInterface('Unknow command '+command+' to Explorer');
        perr(command + " unknow.");
	}
  progressOff();
  return true;
