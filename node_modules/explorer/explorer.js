...
const {runner, exec} = require('child_process');
const fs = require('fs');
...
function synchrosub(flist, source, target, recurse, total) {
  if(flist.length == 0) {
  ...
  for(var i = 0; i < flist.length; i++) {
    ...
    if(stats.isDirectory())  {
      ...
      var sublist = fs.readdirSync(sourcepath);
      total = 0 + synchrosub(sublist, sourcepath, targetpath, true, total);
    }
    else 
    if(stats.isFile())  {
      var newer = false;
...
function synchronize(params) {
  ...
  var total = synchrosub(selection, source, target, true,  0);
  ...
  sendToInterface({"type":"synchro",  "content": total+' file'+s+' copied or updated.' });
  perr(total + " files copied or updated.");
}
...
function loadPage(filename) {
  ...
  if (ext === ".php") {
        ...
        exec(`php "${filename}"`, (error, stdout, stderr) => {
            ...
            // Écrire la sortie dans le fichier temporaire
            fs.writeFileSync(temp, stdout);

            // Charger le fichier temporaire dans la fenêtre
