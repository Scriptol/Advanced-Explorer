...
function loadBoxApp(websocket, fs, params)
{
...
function godir(websocket, filepath, target)
{
...
function mkdir(websocket, params)
{ 
...
function copyRenSub(websocket, params) {
  var source = currpath[params.source];
  var target = currpath[params.target];
  ...
  var targetpath = path.join(target, params.newname);
    ...
    websocket.send(JSON.stringify({
      'app':'"AExplorer',
      'type':'message',  
      'content': "Can't copy a file over itself!"
    }));
		return;
  }
...
/*
  ...
  if(!fs.existsSync(command)) {
    messageToInterface(websocket, command + " archiver not found.");
    return;
  }
...
*/
  ...
  {
    var fname = flist[i];
    var fpath = path.join(orig, fname);
    print("adding " + fpath)
    var fdesc = fs.statSync(fpath);
    print(fdesc)
    if (fdesc && fdesc.isDirectory()) {
      zip.addLocalFolder(fpath, zipfile)
      continue;
    }  
    print('add local')
...
exports.shell = function(websocket, fs, params)
{
  ...
  case 'saveas':
	...
	case 'chdir':  // go to subdir or reload
        ...
        break;
	case 'godir':
        godir(websocket, params.path, target);
        getdir(websocket, fs, currpath[target], params);
        break;
	case 'dirup':
        dirup(target);
  ...
  case 'extract':
	...
	case 'box':
        ...
        break;
  case "boxapp":
        loadBoxApp(websocket, fs, params);
        break;
  case 'run':
