...
function getdir(thepath, params)
{
  ...
  fs.readdir(thepath, function(err, arrfiles)
    ...
    if(arrfiles==undefined) return;  
      ...
      event.sender.send("interface", data)
...
function synchronize(params)
{
  ...
  event.sender.send("interface", JSON.stringify({ "type":"notification","action": "update", "target" : params.target } ));
  event.sender.send("interface", JSON.stringify({ "type":"status",  "content": total + ' file' + s + ' copied or updated.'  } ));
...
function savePrj(params)
{
  ...
  console.log(message);
  if(err)
      event.sender.send("interface", JSON.stringify({"type":'message',  'message': message }));
  else
      event.sender.send("interface", JSON.stringify({"type":'status', 'content' : message }));
}
...
function loadPage(filename) {
  ...
  if(ext == ".md") {
     ...
     var temp = path.join(process.resourcesPath, 'aexplo'+ new String(Date.now()) + ".tmp")
...
function execute(params)
{
  ...
  runner.exec(command, function(err, stdout, stderr) {
      console.log("Terminated. "+ stderr);
      if(err) messageToInterface("Enable to execute " + filename);
  });
}
*/
...
exports.shell = function(params)
{
  ...
  case 'sync':
        ...
        break;    
  case 'copyover':
        copySync(params.path, params.tpath); // do not test if exists
        break;
  case 'copyzip':
        copyZipOver(command);
        break;      
